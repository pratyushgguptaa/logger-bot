...
//util functions for calculations and stuff
...
const add = async (msg, logg) => {
  db.get(msg.author.id + '').then((logs) => {
    if (
      ...
      console.log(`${msg.author.tag}: new Log`)
      ...
      utility.embedder(
        msg,
        `You have successfully logged in for day ${days}\nYay you did it! Remember to log next day 🥰`
      )
      if (!logs['open']) {
      ...
      db.set(msg.author.id + '', obj).then(() => {
        console.log(`Start log by ${msg.author.tag}`)
  ...
  db.get(userId + '').then((logs) => {
    if (typeof logs['info'] !== 'undefined' && logs['info'].length > 0) {
      ...
      db.set(userId + '', obj).then(() => {
        ...
        utility.embedder(
          msg,
          `You didnt have any logs. Logging your first DayOfCode!!`
        )
      })
    }
  })
  ...
  db.get(userId + '')
    .then((logs) => {
      if (setting == 'off') {
        ...
        if (logs['open']) {
          msg.reply(
            `Your privacy settings are currently off. Anyone can tag and see your logs`
          )
        } else {
          msg.reply(
            `Your privacy settings are currently on. No one can tag and check your logs except you`
          )
        }
      }
    })
    .catch((err) => {
      console.log(err)
      msg.channel.send(
        'Sorry you must start your first log in in order to change your privacy settings.'
      )
    })
}
...
const isOpen = async (msg, user) => {
  db.get(user.id + '')
    ...
    .catch((err) => {
      ...
      utility.embedder(
        msg,
        `Sorry mate **${user.username}** is not in our database`
      )
    })
}
    ...
    if (msg.mentions.users.first() === undefined) {
      ...
      db.get(msg.author.id + '')
        ...
        .catch((err) => {
          console.log(err)
          msg.reply(
            `Sorry, some due to some internal error your request could not be processed. Pls try again.`
          )
        })
    } else {
    ...
    if (msg.mentions.users.first() === undefined) {
      ...
      db.get(msg.author.id + '')
        .then((logs) => {
          if (
            ...
            utility.embedder(
              msg,
              `Sorry, **${msg.author.tag}**, you dont have any current logs. Use \`++add {log}\` to start logging.`
            )
          }
        })
        .catch((err) => {
          ...
          utility.embedder(
            msg,
            `${msg.author.tag}, how can i show your streaks when you didnt start any logging.`
          )
        })
    } else {
      user = msg.mentions.users.first()
      ...
      db.get(user.id + '')
        ...
        .catch((err) => {
          ...
          utility.embedder(
            msg,
            `how can i show *${user.username}*'s streaks when they didnt start any logging.`
          )
        })
    }
  }
    ...
    if (msg.mentions.users.first() === undefined) {
      ...
      db.get(user.id + '')
        ...
        .catch((err) => {
          ...
          utility.embedder(
            msg,
            `how can i show *${user.username}*'s profile when they didnt start any logging.`
          )
        })
    }
  }
